...
- **Bug 创建参数别名**: `bug create` 支持用 `--product` 代替 `--productID`，并同步 CLI 帮助与 MCP 参数说明；两种平铺参数同时传入时必须指定同一个产品。

### 🐛 修复 (Fix)

- **多行写入参数**: 完整保留 `--字段名=值` 中的换行、末尾换行和额外等号，避免描述等字段被静默丢弃；格式错误的动态参数现在返回 E2009 并提示正确写法。
- **Bug 创建兼容性**: 将最终解析的 `productID` 同时发送到查询串和请求体，兼容从查询串读取产品 ID 的服务端，并保持 JSON 与标准输入数据的字段优先级。

### 🚀 优化与重构 (Refactor)

- **SDK 依赖**: 升级并锁定 `zentao-api` 至 0.7.0。

### ✅ 测试 (Test)

- **写入参数回归**: 覆盖多种换行、空值、非法参数、Bug 产品别名、参数冲突、JSON 与标准输入优先级，以及任务更新后的读取核实。
- **终端环境隔离**: 测试显式设置并恢复输入输出流的 `isTTY` 状态，避免真实终端环境与测试间状态污染影响断言。

### 📝 文档 (Docs)

- **命令参考**: 补充多行参数引号写法、Bug 产品别名、冲突规则和查询串兼容行为，并同步文档生成脚本及 Markdown / HTML 参考。

### 🤸 技能更新（Skill）

- **zentao-cli**:
  - 新增中英文技能描述、展示名称及维护信息。
  - 补充多行写入参数、Bug 产品别名及 JSON 字段优先级说明。

## 0.3.0-beta.1

### ✨ 新特性 (Feat)

- **模块与版本支持**: 升级 `zentao-api` 至 0.6.9，扩展文档、个人工作、业务需求、问题、风险等模块与命名操作；帮助展示各操作的最低禅道版本，请求时检查服务器兼容性，并提供 E2010～E2012 错误说明。
- **离线能力查询**: 新增 `zentao <module> props` 查看返回对象的字段定义，以及 MCP 工具 `zentao_action_help` 查询操作路径、参数和最低版本，无需登录。
- **技能导出**: `add-skill` 支持通过 `--output <path>` 将两个内置技能及参考资料导出到指定目录。
- **命令补全**: Bash、Zsh、Fish 补全根据注册表生成，覆盖模块操作名、别名、`help`、`props` 和受支持的 Agent 目标。

### 🐛 修复 (Fix)

- **帮助与参数解析**: 后缀 `help` 在鉴权前处理，仅展示动作实际支持的选项；保留多个路径参数的独立名称，无默认列表的模块直接展示帮助，并修复 `ls` 快捷命令的参数与选项传递。
- **批量操作与删除**: 先统一解析位置参数、`--id` 和 `--params` 再确定操作目标；批量结果汇总成功、失败、跳过项及对象数据，失败时返回非零退出码，静默模式保留失败摘要。非交互、JSON、raw 和机器可读模式下删除必须显式传入 `--yes`。
- **认证与账号配置**: 完整环境凭证优先于本地默认账号；修复 MCP 切换账号、Token 更新及退出后的客户端状态，规范服务地址并合并重复账号记录，登录时明确选择密码或 Token 且凭证输入不回显。
- **配置保护**: 校验 `config set` 的值类型；读取或更新损坏配置时报告错误并保留原文件，避免意外清空已有配置。
- **MCP 接入配置**: `add-mcp` 复用 Token 并以 `zentao mcp` 启动，原子写入配置且在 macOS/Linux 上使用 `0600` 权限；修正 Claude Code 配置路径和 OpenCode 的 `environment` 字段，保留 Codex TOML 中的无关表与注释，并拒绝自动重写 JSONC 或无效配置。
- **技能安装与独立可执行文件**: 递归复制技能参考资料，校验导出目标冲突；独立可执行文件内嵌技能内容，脱离源码目录也能安装和导出技能。
- **请求与错误处理**: 写入支持 `--data @-` 和管道 JSON，复用默认与模块级分页大小；raw 模式下的服务端 `status: fail` 正确返回 E2008，避免批量操作误报成功。
...
- **复用 SDK 处理链**: 模块执行器直接使用 `zentao-api request()` 的转换、过滤、搜索、排序、限制与摘取能力，移除重复实现；服务配置请求复用 SDK 的超时、证书校验、错误映射及 24 小时内存缓存。
- **原始输出**: `--format=raw` 直接返回服务端响应原文，不再执行归一化或本地数据处理。
- **兼容性调整**: 移除未完成的 `workspace` 命令、`lang` / `autoSetWorkspace` 配置及未生效的 `--options`；`--version` 与 `-V` 统一只输出 CLI 版本，查看本地缓存的服务器信息使用 `zentao version`。
- **启动与升级**: 移除启动时的后台更新检查，使用 `zentao upgrade` 主动检查 npm 最新稳定版本；非交互升级需要显式传入 `--yes`。
- **Node.js 分发**: npm 构建将依赖保留为外部包，清理旧构建产物，安装后仅需 Node.js 18 及以上版本；开发依赖统一使用 Bun 锁文件。

### ✅ 测试 (Test)

- **Skill 契约回归**: 新增对 `skills/**/*.md` 命令示例的模块、动作、参数名、必填项与过滤语义校验，并覆盖无登录环境下的帮助命令。
- **命令与 MCP 回归**: 补充批量目标解析、删除确认、管道输入、配置保护、账号切换、MCP 配置写入、技能导出与命令补全测试。
- **发布产物验证**: CI 增加 Node.js 18 运行检查、npm 包内容检查，以及 Node.js 和独立可执行文件的技能导出一致性验证；发布前执行类型检查、测试、构建与版本检查。

### 📝 文档 (Docs)

- **命令参考**: 基于注册表生成完整 Markdown 命令参考与 HTML 阅读器，支持搜索命令和参数，并按模块、操作类型筛选。
- **产品与文档站**: 新增 Pixel Tao 品牌首页、Agent 接入指南、站内文档、章节全文搜索和命令浏览器，统一浅深主题与品牌资源。
- **使用说明**: 更新模块范围、最低版本、认证优先级、批量结果、数据处理和 MCP 配置说明，移除已废弃的工作区用法。

### 🤸 技能更新（Skill）

- **zentao-cli**:
  - 精简主技能，拆分写入与数据输出参考；对齐当前模块 schema、必填参数、命名操作、分页和过滤语义，补充技能安装与导出说明。
  - 明确凭证来源、版本兼容性、自动补全边界、写入授权及结果核实规则。
- **zentao-tour**:
  - 重写开场、角色导航与五类角色指南，按用户目标推进，支持无需登录的概念讲解和离线帮助。
  - 更新真实环境就绪检查、版本降级、分页统计、状态流转与演示数据清理规则，区分本地账号记录和实际连接结果。
