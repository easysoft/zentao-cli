...
Error(E1006): 未找到指定的用户配置，请先使用 `zentao login -s <zentao_url> -u <account> -p <password> -t <token>` 登录
...

{
    "error": {
        "code": "1006",
        "message": "未找到指定的用户配置，请先使用 `zentao login -s <zentao_url> -u <account> -p <password> -t <token>` 登录"
    }
}
```
